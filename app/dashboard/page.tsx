...
export default function DashboardPage() {
  ...
  const deleteBulkAnalyses = async () => {
    ...
    if (
      !confirm(
        `Êtes-vous sûr de vouloir supprimer ${count} analyse${
          count > 1 ? "s" : ""
  ...
  const analysesWithMargin = analyses.map((analysis) => {
    const totalCost =
      (analysis.unitPrice || 0) +
      (analysis.shippingCost || 0) +
      (analysis.brandingCost || 0);
  ...
  const categories = Array.from(
    new Set(analysesWithMargin.map((a) => a.category))
  );
  ...
  const avgScore =
    analyses.length > 0
      ? analyses.reduce((acc, a) => acc + (a.finalScore || 0), 0) /
        analyses.length
      : 0;

  const avgMargin =
    analysesWithMargin.length > 0
      ? analysesWithMargin.reduce(
          (acc, a) => acc + (a.grossMarginPercentage || 0),
          0
        ) / analysesWithMargin.length
      : 0;
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="text-center py-20">
            ...
            <Button
              ...
              className="bg-black text-white hover:bg-gray-800 font-medium tracking-wide"
            >
              Réessayer
            </Button>
          </div>
          ...
          >
            <div className="flex items-center justify-between">
              <div>
                ...
                >
                  {analysesWithMargin.length > 0 ? Math.round(avgMargin) : 0}
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-light text-purple-900">
                  {(totalRevenuePotential / 1000).toFixed(0)}
                  <span className="text-lg">K€</span>
                </p>
                <p className="text-xs text-purple-600 mt-1 font-light">
          ...
          <div className="mb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {topPerformers.map((product, index) => (
                <Card
                  key={product.id}
                  className="p-6 bg-white border border-gray-200 hover:border-gray-300 transition-all duration-300"
                >
                  ...
                  <div className="flex justify-between text-xs text-gray-500 font-light">
                    <span>
                      Marge: {Math.round(product.grossMarginPercentage || 0)}%
                    </span>
            ...
            <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-end justify-between">
              ...
              <div className="w-full sm:w-auto">
                <div className="flex items-center gap-3 p-4 border border-gray-300 rounded-lg h-12 sm:h-auto">
                  ...
                  <label
                    htmlFor="selectAll"
                    className="text-sm sm:text-xs font-medium text-gray-700 select-none"
                  >
                    {selectedItems.length > 0
        ...
        <div className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
          {filteredAndSortedAnalyses.length === 0 ? (
            ...
            filteredAndSortedAnalyses.map((analysis) => (
              <Card
                key={analysis.id}
                className={`overflow-hidden hover:border-gray-300 transition-all duration-300 bg-white group relative border ${
                  selectedItems.includes(analysis.id)
              ...
              >
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6 ml-8 sm:ml-6">
                    <div
                      className={`border rounded-lg p-4 min-h-[60px] sm:min-h-0 flex flex-col justify-center transition-all duration-200 ${
                        (analysis.grossMarginPercentage || 0) >= 50
                          ? "border-emerald-200 bg-emerald-50"
                          : (analysis.grossMarginPercentage || 0) >= 30
                          ? "border-blue-200 bg-blue-50"
                          : (analysis.grossMarginPercentage || 0) >= 15
                    ...
                    >
                      ...
                      >
                        {analysis.grossMarginPercentage
                          ? `${Math.round(analysis.grossMarginPercentage)}%`
                          : "N/A"}
                      </p>
                    </div>
                    <div className="border border-gray-200 rounded-lg p-4 min-h-[60px] sm:min-h-0 flex flex-col justify-center">
                      ...
                      <p className="text-lg font-light text-black">
                        {analysis.monthlySearchVolume
                          ? `${(analysis.monthlySearchVolume / 1000).toFixed(
                              1
                            )}K`
                          : "N/A"}
                      </p>
                  ...
                  <div className="space-y-4 mb-6 ml-8 sm:ml-6">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-20 h-1 bg-gray-200 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all duration-300 ${
                              (analysis.googleTrends12MonthAverage || 0) >= 70
                                ? "bg-emerald-500"
                                : (analysis.googleTrends12MonthAverage || 0) >=
                                  50
                                ? "bg-blue-500"
                                : (analysis.googleTrends12MonthAverage || 0) >=
                                  30
                        ...
                        <span className="text-sm font-light min-w-[32px] text-right text-black">
                          {Math.round(analysis.googleTrends12MonthAverage || 0)}
                        </span>
                      </div>
                    </div>
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-3 h-3 rounded-full ${
                            getPerformanceIndicator(analysis.finalScore ?? 0)
                              .color
                          }`}
                        ></div>
                        <span
                          className={`px-3 py-2 rounded-full text-sm font-medium min-w-[70px] text-center ${getScoreBadgeColor(
